...
CALIBRE_LIBRARY=~/Local/CalibreLibrary/
...
cp $EMBLEMS_SOURCE_PATH/* $IMAGES_TARGET_PATH/ || true
find $IMAGES_TARGET_PATH -type f -not -name "Book_*" -not -name ".DS_Store" -exec rename -e 's/(.*)\/(.*)/$1\/calibre_emblems_$2/' {} \; || true
find $IMAGES_TARGET_PATH -type f -not -name "Book_*" -not -name ".DS_Store" -exec mogrify -resize 25x25\> {} \; || true

# Push to Anki is handled by `anki-to-md regenerate-deck` after this script exits.
